        ...
        <div class="rounded-2xl border border-[#1F1F1F] bg-[#111111] p-6">
            <form method="POST" action="{{ route('departments.update', $department) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="mt-2 block w-full rounded-lg border border-[#1F1F1F] bg-[#0A0A0A] text-white placeholder:text-[#71717A] focus:border-[#DC2626] focus:ring-[#DC2626]">{{ old('description', $department->description) }}</textarea>
